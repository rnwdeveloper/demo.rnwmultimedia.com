...

	 {
	    ...
	    // $query ="select * from lead_list where `referred_to`='$name' AND `next_action_date` LIKE '$cutim%' AND `followup_status_red`='red'";
	 	...
	 	if($logtype == 'Counselor'){
	    	...
	    	$query ="select distinct next_followup_date from lead_list where `referred_to`='$name'";

	    }

	    else if($logtype == 'Admin' || $logtype == 'Super Admin') {

	    	$query ="select distinct next_followup_date from lead_list";
...

		{
			...
			$data[] = substr($query2['next_followup_date'],0,10);
		...
		echo json_encode(array('followup_date'=>$data1));
