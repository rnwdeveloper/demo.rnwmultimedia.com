      ...
      <div class="section-body">
         <div class="row">
            <div class="col-12 d-flex justify-content-between">
               <h6 class="page-title text-dark mb-3">Followup Mode</h6>
               ...
               <div class="card">
                  <div class="card-header d-flex justify-content-between income-wrapper">
                     <div class="table-right-content text-right w-100">
                        <button class="btn btn-info"  data-toggle="modal" data-target=".create-channle" onclick="resetval()"><span data-toggle="tooltip" data-placement="bottom" title="Create Followup Mode"><i class="fa fa-plus"></i></span></button>  
                     </div>
                  </div>
                  <div class="card-body">
                     <div class="table-responsive">
                        <table class="table table-striped income-table" id="table1">
                           ...
                           <tbody>
                              <?php $sno=1; foreach($list_followup_mode as $cl) { ?>
                              <tr>
                                 <td><?php echo $sno; ?></td>
                                 <td>
                                 <?php echo $cl->follow_up_mode_name; ?>                     
                                 </td>
                                 <td>
                                 <?php echo $cl->addBy; ?>
                                 </td>
...
<div class="modal fade create-channle" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel"
          aria-hidden="true">
          <div class="modal-dialog modal-sm">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="mySmallModalLabel">Create Channel</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
              <div class="modal-body">
              <form enctype="multipart/form-data" class="document-createmodal" method="post" name="follow_up_mode_add" id="follow_up_mode_add">
              <input type="hidden" name="follow_up_mode_id" id="follow_up_mode_id" class="form-control">
               <div class="form-row">
                  <div class="form-group col-md-12">
                     <label for="inputEmail4">Followup Mode Name:</label>
                     <input class="form-control" type="text" name="follow_up_mode_name" id="follow_up_mode_name" placeholder="Enter Followup Mode Name" required value="">
                  </div>
               </div>
               <div class="form-row">
               <div class="form-group col-md-12">
...
<script>
      $("#follow_up_mode_add").validate({
        ...
        submitHandler: function() {
          ...
          var form = $('#follow_up_mode_add')[0];
          var data = new FormData(form);

          $.ajax({
            type: "POST",
            enctype: 'multipart/form-data',
            url: "<?php echo base_url(); ?>LeadlistController/ajax_followupmode",
            type: "post",
            data: data,
            processData: false,
            contentType: false,
      ...
      function upd(follow_up_mode_id) {
        $.ajax({
          url: "<?php echo base_url(); ?>LeadlistController/get_record_followupmode",
          type: "post",
          data: {
            'follow_up_mode_id': follow_up_mode_id
          },
          success: function(resp) {
            $(".create-channle").modal();
            var data = $.parseJSON(resp);

            var follow_up_mode_id = data['single_record'].follow_up_mode_id;
            var follow_up_mode_name	 = data['single_record'].follow_up_mode_name;
         
            $('#follow_up_mode_id').val(follow_up_mode_id);
            $('#follow_up_mode_name').val(follow_up_mode_name);
      ...
      function status_upd(follow_up_mode_id, status) {
        $.ajax({
          url: "<?php echo base_url(); ?>LeadlistController/update_status",
          type: "post",
          data: {
            'id': follow_up_mode_id,
            'status': status,
            'table': 'list_follow_up_mode',
            'field': 'followup_status',
            'check_field': 'follow_up_mode_id'
          },
          success: function(resp) {
            var data = $.parseJSON(resp);
      ...
      function remove(follow_up_mode_id) {
