      ...
      <div class="section-body">
         <div class="row">
            <div class="col-12 d-flex justify-content-between">
               <h6 class="page-title text-dark mb-3">Lead Followup Sub-Status</h6>
               ...
               <div class="card">
                  <div class="card-header d-flex justify-content-between income-wrapper">
                     <div class="table-right-content text-right w-100">
                        <button class="btn btn-info"  data-toggle="modal" data-target=".create-followupmode" onclick="resetval()"><span data-toggle="tooltip" data-placement="bottom" title="Followup Mode"><i class="fa fa-plus"></i></span></button>  
                     </div>
                  </div>
                  <div class="card-body">
                     <div class="table-responsive">
                        <table class="table table-striped income-table" id="table1">
                           ...
                           <tbody>
                           <?php $sno=1; foreach($substatus_list as $cl) { ?>
                              <tr>
                                 <td><?php echo $sno; ?></td>
                                 <td>
                                 <?php echo $cl->followup_status_id; ?>                     
                                 </td>
                                 <td>
                                 <?php echo $cl->sub_type_name; ?>                     
                                 </td>
                                 <td>
                                 <?php echo $cl->addBy; ?>
                                 </td>
...
<!-- basic modal -->
<div class="modal fade create-followupmode" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
   aria-hidden="true">
   <div class="modal-dialog" role="document">
   <div class="modal-content">
      <div class="modal-header">
         <h5 class="modal-title" id="exampleModalLabel">Create Followup Sub-Status</h5>
         <button type="button" class="close" data-dismiss="modal" aria-label="Close">
         <span aria-hidden="true">&times;</span>
         </button>
      </div>
      <div class="modal-body">
      <form enctype="multipart/form-data" class="document-createmodal" method="post" name="followup_mode_add" id="followup_mode_add">
      <input type="hidden" name="subtype_id" id="subtype_id" class="form-control">
      <div class="form-row">
         <div class="form-group col-md-12">
         <label for="pwd">Lead Followup Status:</label>
            <select name="followup_status_id" id="followup_status_id" class="form-control">
               <option value="teting">Select Status</optin>
                  <?php foreach($status_list as $cl) { ?>
               <option value="<?php echo $cl->followup_type_name; ?>"><?php echo $cl->followup_type_name; ?></optin>
                  <?php } ?>
            </select>
         </div>   
         <div class="form-group col-md-12">
            <label for="inputEmail4">Lead Followup Sub-Status:</label>
            <input class="form-control" type="text" name="sub_type_name" id="sub_type_name" placeholder="Enter Source Name" required value=""> 
         </div>              
      </div>
      <div class="form-row">
         <div class="form-group col-md-12">
   ...
   <script>
      $("#followup_mode_add").validate({
        ...
        submitHandler: function() {
          ...
          var form = $('#followup_mode_add')[0];
          var data = new FormData(form);

          $.ajax({
            type: "POST",
            enctype: 'multipart/form-data',
            url: "<?php echo base_url(); ?>LeadlistController/ajax_folloupsubstatus",
            type: "post",
            data: data,
            processData: false,
            contentType: false,
      ...
      function upd(subtype_id) {
        $.ajax({
          url: "<?php echo base_url(); ?>LeadlistController/get_record_folloupsubstatus",
          type: "post",
          data: {
            'subtype_id': subtype_id
          },
          success: function(resp) {
            $(".create-followupmode").modal();
            var data = $.parseJSON(resp);

            var subtype_id = data['single_record'].subtype_id;
            var sub_type_name	 = data['single_record'].sub_type_name;
            var followup_status_id	 = data['single_record'].followup_status_id;
         
            $('#subtype_id').val(subtype_id);
            $('#sub_type_name').val(sub_type_name);
            $('#followup_status_id').val(followup_status_id);
      ...
      function status_upd(subtype_id, status) {
        $.ajax({
          ...
          data: {
            'id': subtype_id,
            'status': status,
            'table': 'bulk_lead_followup_subtype',
            'field': 'subtype_status',
            'check_field': 'subtype_id'
          },
          success: function(resp) {
            var data = $.parseJSON(resp);
