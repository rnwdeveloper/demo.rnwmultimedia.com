              ...
              <div class="col-12">
                <div class="card">
                  <div class="card-header d-flex justify-content-between income-wrapper">
                    ...
                    <?php $total = 0;for($i=0; $i<sizeof($Outstanding_Fees_list); $i++) {  
                         $total = $total + $Outstanding_Fees_list[$i]->due_amount;
                    }?>
                        <div class="d-flex justify-content-between">
                        <h4>Outstanding Fees Record : <span class="text-danger"><?php setlocale(LC_MONETARY, 'en_IN'); echo @money_format('%!i',$total);  ?> 
                         /-₹</span></h4>
                        </div>  
                    ...
                    <a href="#" class="btn btn-info text-white" data-toggle="modal"
                      ...
                      <span><i class="fas fa-filter mr-1" data-toggle="tooltip" data-placement="bottom" title="Filter"></i></span> 
                      </a>
                      <a href="<?php echo base_url(); ?>Account/export_Outstanding" class="btn btn-info text-white" tabindex="0" aria-controls="tableExport" id=""><span data-toggle="tooltip" data-placement="bottom" title="Excel"><i class="far fa-file-excel mr-1"></i></span></a>
                      <!-- <button class="btn btn-info dropdown-toggle" tabindex="0" aria-controls="tableExport" data-toggle="dropdown"
                        aria-haspopup="true" aria-expanded="false"><span><i class="fas fa-tasks mr-1" data-toggle="tooltip" data-placement="bottom" title="Task"></i></span></button>
                  ...
                  <div class="card-body">
                    <div class="table-responsive">
                      <table class="table table-striped income-table table-bordered" id="table1">
                        ...
                        <tbody>
                          <?php $sno=1; foreach($Outstanding_Fees_list as $val) { ?>
                          <tr>
                            <td><?php echo $sno; ?></td>
                            ...
                            <td>
                              ...
                              <?php foreach($admisison_process_data as $row) { if(in_array($row->admission_id,$admids)) { ?>
                                <?php $branch_ids = explode(",",$row->branch_id);
                                   foreach($list_branch as $ba) { if(in_array($ba->branch_id,$branch_ids)) {  echo $ba->branch_name; }  } ?>
                            ...
                            <td><?php echo $val->due_amount; ?></td>
                            <td>
                              <a href="<?php echo base_url(); ?>Account/view_admission?action=Qwx&Trl=<?php echo @$val->admission_id; ?>" class="bg-primary action-icon text-white btn-primary"><i class="fas fa-eye"></i></a>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="post" action="<?php  echo base_url(); ?>Account/Outstanding_Fees">
         <div class="form-row">
        ...
        <input type="text" class="form-control" name="" value="" placeholder="Amount">
         ...
         <div class="text-right">
          ...
          <a class="btn btn-light text-dark" href="<?php echo base_url(); ?>Account/Outstanding_Fees">Reset</a>  
        </div>
        </form>
        </div>
      </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
         ...
         <div class="form-row">
          ...
          <select id="inputState" class="form-control" id="filter_branch">
        ...
        <div class="form-group col-md-3">
          <label for="inputState">Course</label>
          <select id="inputState" class="form-control" id="filter_course">
            <option value="">Select Course</option>
           <?php foreach($list_course as $lp) { ?>
            <option <?php if(@$filter_course==$lp->course_id) { echo "selected"; } ?> value="<?php echo $lp->course_id; ?>"><?php echo $lp->course_name; ?></option>
            <?php } ?>
         ...
         <div class="form-group col-md-3">
          <label for="inputState">Package</label>
          <select id="inputState" class="form-control" id="filter_package">
            <option value="">Select Package</option>
           <?php foreach($list_package as $lp) { ?>
            <option <?php if(@$filter_package==$lp->package_id) { echo "selected"; } ?> value="<?php echo $lp->package_id; ?>"><?php echo $lp->package_name; ?></option>
            <?php } ?>
        ...
        <div class="form-group col-md-3">
         ...
         <div class="form-group col-md-3">
          ...
          <input type="text" class="form-control datepicker" name="filter_from_date" value="<?php if(!empty($filter_from_date)) { echo @$filter_from_date; } ?>">
        </div>
         <div class="form-group col-md-3">
          <label>To Date</label>
          <input type="text" class="form-control datepicker" name="filter_to_date" value="<?php if(!empty($filter_to_date)) { echo @$filter_to_date; } ?>">
        </div>
        <div class="form-group col-md-3">
          <label>Greater Than Amount</label>
          <input type="text" class="form-control" name="" value="" placeholder="Amount">
         ...
         <div class="text-right">
          <!-- <button class="btn btn-success mr-1" type="submit" name="filter_overdue_fees">Filter</button> -->
          <input type="submit" class="btn btn-success" value="Filter" name="filter_overdue_fees">
          <a class="btn btn-danger" href="<?php echo base_url(); ?>Account/overdue_fees">Reset</a>  
        </div>
        </form>
        </div>
      </div>
