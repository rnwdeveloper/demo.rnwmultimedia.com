...
<div class="col-md-12">
  <div class="card">
    <div class="card-body pl-0 pr-0">
    	<div id="admission_terminated_msg"></div>
    ...
    <?php  $addby =  $_SESSION['Admin']['username']; date_default_timezone_set("Asia/Calcutta");
		$terminate_date = date('d-m-Y');?>
    <input type="hidden" class="form-control" name="addby" id="addby" value="<?php if(isset($addby)){ echo $addby; } ?>"/>
  	 <div class="form-group">
        <label>Terminated Date :<span style="color: red;">*</span></label>
        <input type="text" class="form-control" id="terminate_date" value="<?php echo $terminate_date; ?>">
      </div>
      <div class="form-group">
        <label>Status :<span style="color: red;">*</span></label>
          ...
          <?php foreach($list_dropdown_adm as $ad) { ?>
            <option <?php  if($ad->d_adm_id=="2") { echo "selected"; } ?> value="<?php echo $ad->d_adm_id; ?>"><?php echo $ad->name; ?></option>
        <?php } ?>
     </select>
      </div>
       <div class="form-group">
      ...
      <input type="submit" class="btn btn-primary" id="terminate_adm" value="Submit">
    </div>
  </div>
</div>
<script src="<?php echo base_url(); ?>dist/assets/bundles/izitoast/js/iziToast.min.js"></script>
  ...
  <script>
        var picker = new Lightpick({
          field: document.getElementById('terminate_date'),
...
success: function(resp)
{
  ...
  if(ddd == '1')
  {
      $('#admission_terminated_msg').html(iziToast.success({
                                                          title: 'Success',
                                                          timeout: 2500,
                                                          message: 'HI! Finally Your Admission Is Terminated.',
                                                          position: 'topRight'
                                                        }));
                                                        setTimeout(function() {
                                                            location.reload();
  ...
  {
      $('#admission_terminated_msg').html(iziToast.error({
                                                          title: 'Canceled!',
                                                          timeout: 2500,
