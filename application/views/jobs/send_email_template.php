...
.rating-stars ul > li.star.selected > i.fa {
 ...
 <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <!-- Main content -->
 <a href="javascript:add_category()"  style="margin-left:50px;font-size:20px; position: fixed; bottom:20px; right:20px; padding:10px; background-color: #337AB7; color:white; z-index:10000">Add Email Template</a>
                ...
                <tbody>
                  <?php foreach($template_data as $pos) { ?> 
                    <tr>
                        ...
                        <td><?php echo $pos->SendEmail_template_id; ?></td>
                        <td><?php echo $pos->E_template_name; ?></td>
                        <td><?php echo $pos->E_template_message; ?></td>
                        <td><?php echo $pos->created_at; ?></td>
                        <td><?php echo $pos->created_by; ?></td>

                        <td><a href="javascript:delete_job(<?php echo $pos->SendEmail_template_id; ?>)" style="color:red"><i class="fa fa-trash" aria-hidden="true"></i></a>
                          <a href="javascript:update_job(<?php echo $pos->SendEmail_template_id; ?>)"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a></td>
                    </tr>
                  <?php } ?>
  ...
  <div class="modal-dialog" style="width:600px !important;">
    <div class="modal-content">
      <div class="modal-header">
        <h3 style="font-size: 24px; color: #17919e; text-shadow: 1px 1px #ccc;"><i class="fa fa-folder"></i> Add Email Template</h3>
      </div>
      <div class="modal-body" id="">
        <div id="result_job"></div>
     <form method="post" name="add_sendemail_template" id="add_sendemail_template">  
      <table class="table table-bordered table-striped">
          <thead >
            <tr>
              <td>Template Name</td>
              <td><input type="text" name="E_template_name" id="E_template_name" class="form-control">
              </td>
              <input type="hidden" name="SendEmail_template_id" id="SendEmail_template_id">
            </tr>

            <tr>
              <td>Template Message</td>
              <td>
                <textarea name="E_template_message" id="E_template_message" class="form-control"></textarea>
              </td>
            </tr>
            <tr>
              <td></td>
...
$( "#add_sendemail_template" ).validate({
  rules: {
    w_template_name: {
      required: true,
    },
  ...
  submitHandler: function () {
       ...
       var formdata =  $('#add_sendemail_template').serialize();
...
function update_job(templateId){
  $.ajax({
     url : "<?php echo base_url(); ?>FormController/update_email_template",
     type : "post",
     data:{
       'template_E_id' : templateId
     },
     success:function(resp)
     {
      $("#show_modal").modal();
      var data = $.parseJSON(resp);
      var E_template_name = data['single_record'].E_template_name;
      var E_template_message = data['single_record'].E_template_message;
      var template_id = data['single_record'].SendEmail_template_id;
      $('#E_template_name').val(E_template_name);
      $('#E_template_message').val(E_template_message);
      $('#SendEmail_template_id').val(template_id);
 ...
 function delete_job(id)
  {
    ...
    if(conf){
        $.ajax({
          url : "<?php echo base_url(); ?>FormController/delete_email_template",
          type :"post",
          data:{
            'SendEmail_template_id' : id
          },
