...
<main  class="main_content d-inline-block">
  ...
  <div class="card-body">
    ...
    <p class="card-text">
      ...
      <div class="col-sm-2">
            ...
            <span><b>Branch :</b> <?php $branch_ids = explode(",",$admission->branch_id);   
  ...
  <div class="card-header">
    <ul class="nav nav-tabs card-header-tabs">
      ...
      <li class="nav-item">
        <a class="nav-link active" href="<?php echo base_url(); ?>AddmissionController/admcourses?action=edit&id=<?php echo @$admission->admission_id; ?>"><b>Courses</b></a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="<?php echo base_url(); ?>AddmissionController/admremarks?action=edit&id=<?php echo @$admission->admission_id; ?>"><b>Remarks</b></a>
      </li>
    </ul>
  </div>
  <div class="card-body">
    <!-- <h5 class="card-title">Special title treatment</h5> -->
    <?php if(empty($admission->course_id)) {?>
    <label><b>Course Package :</b> <?php $branch_ids = explode(",",$admission->package_id);   
                        foreach($list_package as $row) { if(in_array($row->package_id,$branch_ids)) {  echo $row->package_name; }  } ?></label>
                      <?php } ?>
    <?php if(empty($admission->package_id)) {?>
    <label><b>Single Course :</b> <?php $branch_ids = explode(",",$admission->course_id);   
                  ...
                  <tr class="td1">
                   ...
                   <?php echo $sno; ?>
                    ...
                    <td>
                      ...
                      $branch_ids = explode(",",$admission->batch_id);   
                        foreach($list_batch as $row) { if(in_array($row->batch_id,$branch_ids)) {  echo $row->batch_name; }  } ?>
                    </td>
                    <td>
                     <a type="button"  data-toggle="modal" data-target="#exampleModals" onclick="return get_shiningsheet_record(<?php echo $val->courses_id;?>,<?php echo $val->admission_courses_id; ?>)">
                            <i class="fa fa-eye" style="font-size:25px;color:#0b527e;"></i>
                          </a>
                    </td>
                    <td>
                      <a href="<?php echo base_url(); ?>AddmissionController/GeneratePdf?action=edit&id=<?php echo $val->admission_courses_id; ?>,<?php echo $val->courses_id;?>">
...
<script type="text/javascript">
 ...
 function get_pdf_Record(course_id='',admission_courses_id=''){
    $.ajax({
      url : "<?php echo base_url(); ?>AddmissionController/GeneratePdf",
      type:"post",
      data:{
        'course_id':course_id , 'admission_courses_id':admission_courses_id
      },
