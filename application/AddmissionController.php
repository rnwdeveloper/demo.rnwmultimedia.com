	...
	public function seat_assign()
	{
		...
		{	
			...
			if(isset($data['seat_id']) && !empty($data['seat_id']))
			{
				...
				$bdata = $this->cm->filter_data("seat_batch","seat_id",$data['seat_id']);$this->cm->view_all("seat_batch");		
   ...
   public function GeneratePdf()
   {
	...
	public function assigned_multistudent()
	{
	    ...
	    foreach ($adm_data as $key) {
	    	...
	    	$a_id = strtr(base64_encode($key->admission_id), '+/=', '._-');



	    	$g_id = strtr(base64_encode($key->gr_id), '+/=', '._-');



	    	$cp_id = strtr(base64_encode($key->courses_id), '+/=', '._-');
	    	...
	    	$emailconfigration = $this->cm->view_all('email_settings');
		        ...
		        $from = "rnwstudentcorner@gmail.com";
	...
	public function firstProcess()
	{
		...
		else
		{
		    ...
		    $record = array('lead_list_id'=>$lead_list_id,'demo_id'=>$demo_id,'shining_sheet_id'=>$shining_sheet_id,'gr_id'=>$g_id,'admission_number'=>$admission_number,'enrollment_number'=>$enrollment_number,'admission_date'=>$admission_date,'mobile_no'=>$data['mobile_no'],'alternate_mobile_no'=>$data['alternate_mobile_no'],'email'=>$data['email'],'source_id'=>$data['source_id'],'first_name'=>$data['first_name'],'surname'=>$data['surname'],'addby'=>$data['addby'],'branch_id'=>$data['branch_id'],'year'=>$year,'department_id'=>$depart,'subdepartment_id'=>$subdepart,'type'=>$type,'course_id'=>$co,'package_id'=>$co_p,'faculty_id'=>$facultyid,'batch_id'=>$batch,'tuation_fees'=>$data['tuation_fees'],'registration_fees'=>$data['registration_fees'],'no_of_installment'=>$data['no_of_installment'],'father_mobile_no'=>$father_mobile_no,'mother_mobile_no'=>$mother_mobile_no,'admission_status'=>$admission_status,'rnw1_no'=>$rnw1_no,'rnw2_no'=>$rnw2_no,'rnw3_no'=>$rnw3_no,'rnw4_no'=>$rnw4_no,'RW1B'=>$RW1B,'rw1mm'=>$rw1mm,'clg'=>$clg);
	...
	public function fetch_course_wise_faculty()
	{
	  ...
	  $faculty = $this->admi->view_all('faculty');
	    ...
	    foreach($faculty as $dn)
		{
			...
			if($flag==1)
			{
                ...
                <option <?php if($data['branch_id']==$dn['branch_ids']) { ?>  value="<?php echo $dn['faculty_id']; ?>"><?php echo $dn['name']; ?></option>
...
public function fetch_package_wise_faculty()
{
...
public function Get_faculty()
{
	...
	public function fetch_batch_data()
	{
		 ...
		 $batch = $this->admi->get_batches_all('batches','faculty_id',$id);
    ...
    public function search_data()
...

	   {
	   	  ...
	   	  $record['get_install'] =  $this->cm->get_packageName_installment('package','package_id',$pId);
...

	   {
	   	 ...
	   	 $courseData = $this->input->post('courseId');
	   	  ...
	   	  $record['get_install'] =  $this->admi->get_courseSingle_installment('course','course_id',$cId);
	   ...
	   public function GetInstall_with_registration(){
	   		...
	   		$record['data'] =  array('tution_fee'=>$this->input->post('tution_fee'),
...

	{
...

		{
		  ...
		  $recp["all_record"] = array('status'=>1,"msg"=>"This Admission Holded");
...

   {
	 ...
	 public function Batch_add()

  	{
...

		{
			...
			$faculty_id = implode(",",@$data['faculty_id']);

		}



		$record = array('batch_name'=>$data['batch_name'],'batch_code'=>$data['batch_code'],'branch_id'=>$data['branch_id'],'faculty_id'=>$faculty_id,'course_id'=>$data['course_id'],'created_bye'=>$data['created_bye']);
	...
	public function Batch_update()
	{
		...
		if(!empty($data['batches_id']))
		{
			...
			if(@$old_record['re']->created_bye == @$data['created_bye']){

				$up_created_bye = $data['created_bye']."&#nochange";  

			}else{

				$up_created_bye = $data['created_bye']."&#change";

			}

			date_default_timezone_set("Asia/Calcutta");

		    $batch_upd_date = date('m/d/Y h:i A');
			...
			$record = array('batch_name'=>$data['batch_name'],'batch_code'=>$data['batch_code'],'branch_id'=>$data['branch_id'],'faculty_id'=>$faculty_id,'course_id'=>$data['course_id'],'created_bye'=>$data['created_bye']);
	         ...
	         $re = $this->admi->update_batch('batches',$record,'batches_id',$data['batches_id']);

			if($re)
			{

				$history_record  = array('batches_id'=>$data['batches_id'],'batch_name'=>$up_batch_name,'batch_code'=>$up_batch_code,'branch_id'=>$up_branch_id,'faculty_id'=>$up_faculty_id,'course_id'=>$up_course_id,'created_bye'=>$up_created_bye,'batch_upd_date'=>@$batch_upd_date);
		...
		{
			$check_ep =  $this->admi->check_record_alerady('batches',$record);

			if($check_ep==0)
			{
				$re =  $this->admi->quick_batch_data('batches',$record);

				if($re)
				{
					$history_record  = array('batches_id'=>@$re,'batch_name'=>$data['batch_name']."&#nochange",'batch_code'=>$data['batch_code']."&#nochange",'branch_id'=>$data['branch_id']."&#nochange",'faculty_id'=>$faculty_id."&#nochange",'course_id'=>$data['course_id']."&#nochange",'created_bye'=>$data['created_bye']."&#nochange",'batch_upd_date'=>@$batch_upd_date);
	...
	public function not_assigned_batch_assign()
	{
		...
		$data = $this->input->post();
		$record = array('batch_id'=>$data['batch_id']);
...

	{
...

		{
		  ...
		  $recp["all_record"] = array('status'=>1,"msg"=>"This Topic Edited  Successfully");
...

	{
...

		{
		  ...
		  $recp["all_record"] = array('status'=>1,"msg"=>"This Topic Deleted  Successfully");
...

	{
		...
		$not_assingned_batch =  $this->admi->get_adm_batches_all('admission_courses','courses_id',$course_id);



		$status_check = 0;

		foreach ($not_assingned_batch as $val) 
			...
			{ 
				...
				{ 
					...
					$this->db->update('admission_process');	

						

					$this->db->set('batch_id', $batches_id);

					$this->db->where('admission_courses_id', $val->admission_courses_id);
	...
	public function gstreceiptsetting()
	{
		...
		if($this->input->get('action')=="delete")
			{
				$re=$this->cm->delete_data("gst_receipt_template","gst_receipt_template_id",$id);

				if($re)
				{
					logdata('gst_receipt_template_id id= '.$id." Deleted");

					redirect('AddmissionController/gstreceiptsetting');	
				}
			...
			else if($this->input->get('action')=="edit")
			{
				...
				$display['select_gst_receipt']=$this->cm->select_data("gst_receipt_template","gst_receipt_template_id",$id);
		...
		if(!empty($this->input->post('submit')))
			{
				...
				unset($data['submit']);
			   	...
			   	@$ins_data['gst_percentage'] = $data['gst_percentage'];

			   	@$ins_data['branch_address'] = $data['branch_address'];

			   	@$ins_data['gst_number'] = $data['gst_number'];
			   	...
			   	@$ins_data['received_by_name'] = $data['received_by_name'];
            		...
            		if(!empty($this->input->post('update_id')))
					{
					   $id = $this->input->post('update_id');
						...
						$re = $this->cm->update_data("gst_receipt_template",$ins_data,"gst_receipt_template_id",$id);
					...
					{
						$re = $this->cm->insert_data("gst_receipt_template",$ins_data);
    	...
    	$display['list_package'] = $this->cm->view_all("package");
		$display['list_source'] = $this->cm->view_all("source");
		$display['list_gst_receipt'] = $this->cm->view_all("gst_receipt_template");
		$display['hod_all'] = $this->cm->view_all_hod_demo("hod");
		$display['faculty_all'] = $this->cm->view_all("faculty");		
		$display['reason_list'] = $this->cm->view_all("cancel_reason_list");
		...
		$this->load->view('header_test',$update);
		$this->load->view('erp/gstreceiptsetting',$display);
	}
