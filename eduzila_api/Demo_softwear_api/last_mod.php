...
if(isset($_POST['email']) && !empty($_POST['email']))
{
	...
	if($num == 1){
				...
				$data['status'] = 1;
				$pall = explode(',',$q2['permission']);
				$a = "SELECT * FROM l_module WHERE m_module_id='".$_POST['m_module_id']."'";
				...
				foreach ($jjall as $key => $value) {
					...
					{
						$value['path'] = "https://demo.rnwmultimedia.com/".$value['controller'].$value['method'];
